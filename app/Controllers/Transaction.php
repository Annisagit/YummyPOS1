...
class Transaction extends BaseController
{
    ...
    {
        $this->data['page_title'] = 'Transaksi | YummyPOS';
        ...
        $this->data['detail_prices'] = $this->detail_transaction_model->select('id_transaksi, menu_yummy.harga as harga, jumlah_produk')->join('stok_menu', 'stok_menu.id_stok = detail_transaksi.id_stok')->join('menu_yummy', 'stok_menu.id_menu = menu_yummy.id_menu')->get()->getResult();
    ...
    public function detail($transaction_id = "")
    {
        ...
        $this->data['transaction_details'] = $this->detail_transaction_model->select('*')->join('stok_menu', 'stok_menu.id_stok = detail_transaksi.id_stok')->join('menu_yummy', 'stok_menu.id_menu = menu_yummy.id_menu')->where(['id_transaksi' => $transaction_id])->get()->getResult();
    ...
    private function insert_transaction($status = "Berhasil", $total_order = 0)
    {
        ...
        for ($i = 1; $i <= (int) $total_order; $i++) {
            ...
            $total_menu = (int) $transaction_detail_data['jumlah_produk'];

            $this->stock_model->where(['id_stok' => $transaction_detail_data['id_stok']])->set(['stok' => $max_stock - $total_menu])->update();
        }
    ...
    private function update_transaction($status = "Berhasil", $total_order = 0, $transaction_id)
    {
        ...
        for ($i = 1; $i <= $total_order; $i++) {
            ...
            $max_stock = (int) $this->request->getPost($i . '-max-stock');
            $total_menu = (int) $insert_detail['jumlah_produk'];

            if (!$this->detail_transaction_model->where($where_detail)->first()) {
                $this->detail_transaction_model->insert($insert_detail);
            } else {
                $this->detail_transaction_model->where($where_detail)->set(['jumlah_produk' => $total_menu])->update();
            }

            $this->stock_model->where(['id_stok' => $insert_detail['id_stok']])->set(['stok' => $max_stock - $total_menu])->update();
        }

        $all_transaction_details = $this->detail_transaction_model->select("id_transaksi, detail_transaksi.id_stok, jumlah_produk, stok_menu.stok")->join('stok_menu', 'stok_menu.id_stok = detail_transaksi.id_stok')->where(['id_transaksi' => $transaction_id])->get()->getResult();
    ...
    public function change_status()
    {
        ...
        if ($transaction_status == "gagal") {
            $all_transaction_details = $this->detail_transaction_model->select("detail_transaksi.id_stok, jumlah_produk, stok_menu.stok")->join('stok_menu', 'stok_menu.id_stok = detail_transaksi.id_stok')->where(['id_transaksi' => $transaction_id])->get()->getResult();
    ...
    public function print_transaction($transaction_id = "")
    {
        ...
        $this->data['transaction_details'] =
            $this->detail_transaction_model->select('*')->join('stok_menu', 'stok_menu.id_stok = detail_transaksi.id_stok')->join('menu_yummy', 'stok_menu.id_menu = menu_yummy.id_menu')->where(['id_transaksi' => $transaction_id])->get()->getResult();
